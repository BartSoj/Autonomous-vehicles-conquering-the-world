...
static void errorLed(void){
  // PB5 - UNO
  // PB7 - Mega 2560
  DDRB |= _BV(7); // make sure led is output
...
static void twiStart(void){
  ...
  //errorLed();
  while(!(TWCR & (1<<TWINT))); //wait for start to be transmitted // TODO: mrv: getting stuck here
  //errorLed();
...
static void twiWriteByte(uint8_t DATA,uint8_t type){
  ...
  //errorLed();
  while (!(TWCR & (1<<TWINT)));   /* wait for transmission */ // TODO: mrv: getting stuck here
  //errorLed();
  if ((TWSR & 0xF8) != type)
    errorLed();
}
